...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-2xl font-display font-bold text-white mb-8">My Services</h1>
    ...
    @if ($servers->isEmpty())
        <div class="glass rounded-2xl p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-dark-300 mb-2">No services yet</h3>
            <p class="text-dark-500 text-sm mb-6">You don't have any services. Browse our products to get started.</p>
                ...
                <a href="{{ route('dashboard.server-detail', $server) }}" class="glass rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-primary-500/30 transition-all group block">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-dark-400">{{ $server->product->name ?? 'N/A' }} &mdash; {{ $server->plan->name ?? 'N/A' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                            ...
                            {{ $server->status === 'pending' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' : '' }}
                            {{ $server->status === 'cancelled' ? 'bg-dark-500/10 text-dark-400 border border-dark-500/20' : '' }}">
