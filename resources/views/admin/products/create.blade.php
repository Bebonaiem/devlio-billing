...
<div class="max-w-lg">
    <form method="POST" action="{{ route('admin.products.store') }}" class="glass rounded-2xl p-6 sm:p-8 space-y-5">
        ...
        <div>
            ...
            <input type="text" name="slug" class="w-full px-4 py-3 rounded-xl input-field text-white placeholder-dark-500 text-sm" value="{{ old('slug') }}">
        </div>
        <div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="per_user_limit" min="0" class="w-full px-4 py-3 rounded-xl input-field text-white text-sm" value="{{ old('per_user_limit') }}">
            </div>
            <div>
                <label class="block text-sm font-medium text-dark-300 mb-2">Stock</label>
                <input type="number" name="stock" min="0" class="w-full px-4 py-3 rounded-xl input-field text-white text-sm" value="{{ old('stock') }}">
            </div>
        </div>
        <div class="flex items-center gap-6">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="hidden" value="1" class="w-4 h-4 rounded border-dark-600 bg-dark-800 text-primary-500 focus:ring-primary-500 focus:ring-offset-0">
                <span class="text-sm text-dark-300">Hidden</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="allow_quantity" value="1" class="w-4 h-4 rounded border-dark-600 bg-dark-800 text-primary-500 focus:ring-primary-500 focus:ring-offset-0">
