    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}" class="glass rounded-2xl p-6 sm:p-8 space-y-5">
        ...
        <div class="flex items-center gap-6">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="w-4 h-4 rounded border-dark-600 bg-dark-800 text-primary-500 focus:ring-primary-500 focus:ring-offset-0">
