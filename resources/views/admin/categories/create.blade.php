...
@section('content')
<div class="max-w-lg">
    <form method="POST" action="{{ route('admin.categories.store') }}" class="glass rounded-2xl p-6 sm:p-8 space-y-5">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="text" name="slug" class="w-full px-4 py-3 rounded-xl input-field text-white placeholder-dark-500 text-sm" value="{{ old('slug') }}" placeholder="Leave blank to auto-generate">
        </div>
        <div>
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                ...
                <input type="number" name="order" value="0" class="w-20 px-3 py-2 rounded-xl input-field text-white text-sm">
            </div>
        </div>
        <button type="submit" class="w-full py-3 px-4 btn-primary text-white font-medium rounded-xl text-sm">Create Category</button>
    </form>
</div>
@endsection