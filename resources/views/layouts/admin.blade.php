...
<body class="bg-dark-950 text-white min-h-screen flex">
    <aside id="sidebar" class="w-64 bg-dark-900/80 backdrop-blur-xl border-r border-white/5 min-h-screen flex-shrink-0 flex flex-col fixed lg:sticky top-0 h-screen z-40 transition-transform -translate-x-full lg:translate-x-0">
        ...
        <nav class="flex-1 p-3 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.orders.index') }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-dark-300 hover:text-white {{ request()->routeIs('admin.orders.*') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                Orders
            </a>
            <a href="{{ route('admin.servers.index') }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-dark-300 hover:text-white {{ request()->routeIs('admin.servers.*') ? 'active' : '' }}">
